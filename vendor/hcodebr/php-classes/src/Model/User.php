...
class User extends Model{
	...
	public static function getForgot($email, $inadmin = true){
		...
		if(count($results) === 0){
			...
			$resultsDois = $sql->select("CALL sp_userspasswordsrecoveries_create(:iduser, :desip)", array(
					":iduser"=>$data["iduser"],
					":desip"=>$_SERVER["REMOTE_ADDR"]
			));

			if(count($resultsDois) === 0){
				
				throw new \Exception("Não foi possível recuperar a senha");
				
			}else{

				$dataRecovery = $resultsDois[0];
	...
	public static function setForgorUsed($idrecovery){
